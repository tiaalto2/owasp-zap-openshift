...
RUN curl -s -k https://raw.githubusercontent.com/zaproxy/zap-admin/master/ZapVersions-dev.xml | xmlstarlet sel -t -v //url |grep -i Linux | wget --no-check-certificate -q --content-disposition -i - -O - | tar zx --strip-components=1 && \
    curl -s -k -L https://bitbucket.org/meszarv/webswing/downloads/webswing-2.3-distribution.zip | jar -x && \
